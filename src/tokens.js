...
const tokens = {
   ...
   *   'createAnotherMint': if should create another mint authority
   ...
   * @inner
   */
  getCreateTokenData(address, name, symbol, mintAmount, pin, options = {}) {
    ...
    const { changeAddress, createAnotherMint, createMelt, nftData } = options;
   ...
   * @inner
   */
  getMintDepositInfo(mintAmount, options = { changeAddress: null, isNFT: false }) {
    const { changeAddress, isNFT } = options;
    const outputs = [];
    const data = wallet.getWalletData();
